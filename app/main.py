...
def static_file(ref: str):
    """The file a `/static/...` ref points at, or None if the ref is not one.

    The approval card is the only place the studio asks a human to judge a
    picture, so it resolves the ref the same way StaticFiles will and refuses
    to leave the mount - a ref that escapes /static is not a picture we own.
    """
    if not ref or not ref.startswith("/static/"):
        return None
    p = (Path(__file__).parent / ref.lstrip("/")).resolve()
    return p if p == STATIC_DIR or STATIC_DIR in p.parents else None


def image_ready(ref: str) -> bool:
    """Is there actually something to LOOK at behind this ref, right now?

    `st['thumb']['ref']` is written the moment the studio decides what the file
    will be called; the bytes land separately. A half-written or missing PNG is
    a 404 (or a broken image) in the browser, which is indistinguishable from
    "no thumbnail" to the human being asked to approve it. So the card asks the
    disk, not the state file.
    """
    p = static_file(ref)
    try:
...
<span style="font-size:12.5px;color:var(--sub)">python -m agent.learn, as a button</span></div></form>"""
        # A lap can finish short: the join publishes what the farm delivered and
        # records the rest as failed. Say so here - a missing shot the page never
        # mentions is the same as a page that lied about it.
        _shots = st.get("shots") or []
        _landed = sum(1 for s in _shots if s.get("url"))
        shots_row = ""
        if _shots and _landed < len(_shots):
            _why = next((s.get("reason") for s in _shots
                         if not s.get("url") and s.get("reason")), "")
            shots_row = (f'<div class="h0s" style="color:var(--sub)">published with '
                         f'{_landed} of {len(_shots)} shots — '
                         f'{len(_shots) - _landed} never rendered'
                         + (f' ({html.escape(str(_why))})' if _why else "") + '</div>')
        return busy_html + f"""
...
<span style="font-size:12.5px;color:var(--sub)">pre-filled from <b>user:prefs</b> — the studio opens with your taste</span>
    ...
    if pend_thumb and not any(a["kind"] == "thumb" for a in st["lineage"]["approvals"]):
        tb = st.get("thumb") or {}
        thumb = tb.get("ref", "")
        gen = "generated from your brief" if tb.get("generated") else "prebaked stand-in"
        # WHAT is being judged: the direction this picture was drawn from, and
        # the 2-4 sticker words printed on it. Approving a thumbnail without
        # them is approving a picture with no brief attached.
        idea = st.get("direction") or (st.get("script") or {}).get("title", "")
        caption = (st.get("hook") or "").strip()
        brief = f'<div class="h0s">from your direction: <b>{html.escape(str(idea))}</b></div>' if idea else ""
        sticker = f'<div class="h0s">caption on the picture: “{html.escape(caption)}”</div>' if caption else ""

        if not image_ready(thumb):
            # NEVER offer Approve when there is nothing to look at. thumb_submit's
            # own docstring puts the picture 20-30s behind the receipt, and a
            # write can fail outright - either way the human must not be asked to
            # judge a blank. No Approve, no Regenerate: there is nothing yet to
            # accept or to replace.
            return busy_html + f"""
<div class="card"><div class="h0">Still drawing your thumbnail…</div>
<div class="h0s">the studio has the brief and the file name — the picture itself is
still landing (~20-30s). This page refreshes itself; Approve appears when there
